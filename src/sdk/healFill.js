...
export async function healFill(page, selector, value, { intent = null } = {}) {
    ...
    if (cachedFix && cachedFix.healed_selector) {
        ...
        try {
            ...
            console.log(`  ⚠️  [healFill] Cached selector stale, re-healing...`);
        }
    }
    ...
    try {
        ...
        if (!healResult.newSelector) {
            console.log(`  💀 [healFill] No fix found for "${selector}"`);
            throw err;
        }
        ...
        if (healResult.confidence >= threshold) {
            ...
            // ── 5. Safe mode ─────────────────────────────────────────
            console.log(`  ⚠️  [healFill] Low confidence (${(healResult.confidence * 100).toFixed(0)}% < ${threshold * 100}%)`);
            ...
            if (safeMode) {
                console.log(`  🔒 [healFill] Safe mode ON — blocking until human confirms.`);
            }
