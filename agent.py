...
class WebHealerAgent:
    def __init__(self, config_path="config.json", scripts_path="scripts.json", heals_path="heals.json"):
    ...
    def ask_gemini(self, broken_selector: str, page_html: str, intent: str) -> str | None:
    ...
    def healing_click(self, page, selector: str, intent: str, script_name: str):
        ...
        try:
            ...
            new_selector = self.ask_gemini(selector, page_html, intent)
    ...
    def scan_page(self, url: str) -> list:
        ...
        # Save to heals.json as requested
        # Note: the prompt asks to save to heals.json structure. 
        # We append these scan fingerprints to the existing cache.
        heals = self.load_heals()
        for fp in elements_data:
            heals.append({
                "original_selector": "scanned-element",
                "healed_to": "scanned-element",
                "method": "scan",
                "confidence": 100,
                "intent": f"Pre-cached fingerprint: {fp['tag']} id={fp['id']} text={fp['text']}",
                "timestamp": fp["timestamp"],
                "script_name": "scan_page",
                "fingerprint": fp
            })
        self._save_json(self.heals_path, heals)
